...
flowchart LR
    ...
    Renderer --> Shader["GLSL"]
    Shader --> Screen["Viewport"]
```
...
3. [`Systems.cpp`](../../src/Systems.cpp)의 `RenderScene::sync`에서 revision 비교를
   찾는다.
4. [`Renderer.cpp`](../../src/Renderer.cpp)의 `Renderer::render`에서 proxy가
   uniform과 draw call로 변환되는 순서를 읽는다.
5. [`basic.vert`](../../shaders/basic.vert)와
   [`basic.frag`](../../shaders/basic.frag)에서 GPU 단계의 입력을 확인한다.

## 실험 과제

큐브 색을 바꾸는 setter를 호출하기 전후로 `RenderScene`의 갱신 횟수를 출력한다.
변경하지 않은 프레임에서는 proxy가 다시 만들어지지 않는지 확인한다.
