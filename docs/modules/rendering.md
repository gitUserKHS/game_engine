...
5. `Renderer::renderToTarget`에서 장면과 선택 wireframe을 off-screen target에
   그리는 순서를 읽는다.
6. [`Application.cpp`](../../src/Application.cpp)의 `drawViewportPanel`에서
   color texture가 `ImGui::Image`로 표시되는지 확인한다.
7. [`basic.vert`](../../shaders/basic.vert)와
...
확인한다. `testPngWriter`는 RGBA readback의 상하 방향 규칙을 확인한다.
